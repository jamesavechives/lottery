...
class EloquentAuthenticationRepository implements AuthenticationContract
{
    ...
    {
        if ($this->auth->attempt($request->only('name', 'password'), $request->has('remember'))) {
            $this->isBannedOrDeactivated($this->auth->user());

            if ($this->auth->user()->confirmed == 0) {
                $user_id = $this->auth->user()->id;
                $this->auth->logout();
                throw new GeneralException('您的账号还未激活. 请 ' . '<a href="' . route('account.confirm.resend', $user_id) . '">点击此处</a>' . ' 来发送一封确认邮件到您的邮箱.');
            }
