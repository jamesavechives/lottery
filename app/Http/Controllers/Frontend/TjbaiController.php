...
use DB;
use Input;
class TjbaiController extends Controller{
	...
	public function data(){
        ...
        $zero_number_arr   =array($last_data[0]->WAN,$last_data[0]->QIAN,$last_data[0]->SHI,$last_data[0]->GE);
        $one_number_arr    =array($last_data[1]->WAN,$last_data[1]->QIAN,$last_data[1]->SHI,$last_data[1]->GE);
        $two_number_arr    =array($last_data[2]->WAN,$last_data[2]->QIAN,$last_data[2]->SHI,$last_data[2]->GE);
        $three_number_arr  =array($last_data[3]->WAN,$last_data[3]->QIAN,$last_data[3]->SHI,$last_data[3]->GE);
        $four_number_arr   =array($last_data[4]->WAN,$last_data[4]->QIAN,$last_data[4]->SHI,$last_data[4]->GE);
        $five_number_arr   =array($last_data[5]->WAN,$last_data[5]->QIAN,$last_data[5]->SHI,$last_data[5]->GE);
        ...
        foreach ($data as $key => $value) {
            //$number_arr = array($value->WAN,$value->QIAN,$value->BAI,$value->SHI,$value->GE);
            //$number_arr = array($value->QIAN,$value->BAI,$value->SHI,$value->GE);

            //$number_arr = array($value->WAN,$value->BAI,$value->SHI,$value->GE);
            $number_arr = array($value->WAN,$value->QIAN,$value->SHI,$value->GE);
            
            $eid = $value->EID;
            $qihao=$value->QIHAO;
            //$number_str = $value->WAN.$value->QIAN.$value->BAI.$value->SHI.$value->GE;
            //$number_str = $value->QIAN.$value->BAI.$value->SHI.$value->GE;

            //$number_str = $value->WAN.$value->BAI.$value->SHI.$value->GE;
            $number_str = $value->WAN.$value->QIAN.$value->SHI.$value->GE;
            ...
            if($key == 0){
               ...
               //Redis::command('HSET', ['yilou', $eid, $expand]);
                ...
                Redis::command('HSET', ['yiloubai', $eid, $expand]);
                
                
            }else{
                //$last_number_arr      = array($data[$key-1]->WAN,$data[$key-1]->QIAN,$data[$key-1]->BAI,$data[$key-1]->SHI,$data[$key-1]->GE);
                //$last_number_arr      = array($data[$key-1]->QIAN,$data[$key-1]->BAI,$data[$key-1]->SHI,$data[$key-1]->GE);
            //$last_number_arr      = array($data[$key-1]->WAN,$data[$key-1]->BAI,$data[$key-1]->SHI,$data[$key-1]->GE);
            $last_number_arr    = array($data[$key-1]->WAN,$data[$key-1]->QIAN,$data[$key-1]->SHI,$data[$key-1]->GE);
                //组选120的值
        ...
        Redis::command('HSET', ['zxtjbai', 'zxtj'.$na, json_encode(${'zxtjn'.$na})]);
        ...
        Redis::command('HSET', ['sjptjbai', 'sjptj'.$nb, json_encode(${'sjptjn'.$nb})]);
           ...
           //Redis::command('HSET', ['sjpjltj', 'sjpjltj'.$nb, json_encode(${'sjpjltjn'.$nb})]);
            ...
            Redis::command('HSET', ['sjpjltjbai', 'sjpjltj'.$nb, json_encode(${'sjpjltjn'.$nb})]);
