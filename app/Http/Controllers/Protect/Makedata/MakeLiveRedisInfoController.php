...
use DB;
use Input;
class MakeLiveRedisInfoController extends Controller{

    public function index($count,$type){

        //链接redis
        $redis = Redis::connection('livearray');

        //获取redis中当前最大的值（最新期数） 没有设为 1
        //($redis->Get('thebig')) ? $big_index = $redis->Get('thebig') : $big_index = 69000;
        //68000
        ...
        $data =  DB::table($type)
             ...
             ************************************************/
            //获取开奖号
            $jihao              = $data[0]->WAN.$data[0]->QIAN.$data[0]->BAI.$data[0]->SHI.$data[0]->GE;
            //获取上一期开奖号
            $last_jihao         = self::last_qihao(($big_index-2),$type);
            //获取上二期开奖号
            $last2_jihao        = self::last_qihao(($big_index-3),$type);
            //获取上三期开奖号
            $last3_jihao        = self::last_qihao(($big_index-4),$type);
            //获取上四期开奖号
            $last4_jihao        = self::last_qihao(($big_index-5),$type);
            //获取上五期开奖号
            $last5_jihao        = self::last_qihao(($big_index-6),$type);
            //获取上六期开奖号
            $last6_jihao        = self::last_qihao(($big_index-7),$type);

            //获取本期静态数组所属值
            $first              = self::first($jihao,'new_arr_1',($big_index-2),$count,$type);
            //上一期静态数组所属值
            $last_first         = self::first($last_jihao,'new_arr_2',($big_index-3),$count,$type);
            //上二期静态数组所属值
            $last2_first        = self::first($last2_jihao,'new_arr_3',($big_index-4),$count,$type);
            //上三期静态数组所属值
            $last3_first        = self::first($last3_jihao,'new_arr_4',($big_index-5),$count,$type);
            //上四期静态数组所属值
            $last4_first        = self::first($last4_jihao,'new_arr_5',($big_index-6),$count,$type);
            //上五期静态数组所属值
            $last5_first        = self::first($last5_jihao,'new_arr_6',($big_index-7),$count,$type);
            //上六期静态数组所属值
            $last6_first        = self::first($last6_jihao,'new_arr_7',($big_index-8),$count,$type);  //第一组列配置结束
            ...
            //生成新的遗漏值并替换
            $one_new                    = self::newylarr(1,5,$one,$first);
            //升降平
            ...
            //距离1
            $jl_arr_new                 = self::newylarr(0,4,$jl_arr,$jl_num);
            //距离升降平
    ...
    // 返回数组第一序列值
    public function first($jihao,$title,$begin,$count,$type){
        //链接redis--zst_set库
        $big_data_redis = Redis::connection('big_data');

        $now_big_data_arr = $big_data_redis -> zRange('bigdata5',0,-1);

        //获取库里面的具体值
        if($big_data_redis -> zCount($title,0,50) > 0){
            $ll_arr = $big_data_redis -> zRange($title,0,-1);
            $data =  DB::table($type)
                -> select('EID','QIHAO','WAN','QIAN','BAI','SHI','GE')
                -> where('EID', '=', ($begin))
                -> get();
            $val = $data[0]->WAN.$data[0]->QIAN.$data[0]->BAI.$data[0]->SHI.$data[0]->GE;
            array_shift($ll_arr);
            array_push($ll_arr,$val);
            $big_data_redis -> ZREMRANGEBYRANK($title,0,$count);
            foreach($ll_arr as $key=> $value){
                $big_data_redis -> zAdd($title,$key,$value);
            }
            $now_arr = $big_data_redis -> zRange($title,0,-1);
        }else{
            for($i = 0; $i <= $count; $i++){
                $data =  DB::table($type)
                    -> select('EID','QIHAO','WAN','QIAN','BAI','SHI','GE')
                    -> where('EID', '=', (($begin-$count)+$i))
                    -> get();

                $val = $data[0]->WAN.$data[0]->QIAN.$data[0]->BAI.$data[0]->SHI.$data[0]->GE;

                $big_data_redis -> zAdd($title,$i,$val);
            }

            $now_arr = $big_data_redis -> zRange($title,0,-1);
        }

        if(in_array($jihao,$now_arr)){
            return 1 ;
        }else{
            $count_arr = array_merge(array_diff($now_big_data_arr,$now_arr),array_diff($now_arr,$now_big_data_arr));
            foreach($count_arr as $val){
                if(preg_match('/8/', $val) == 1 || preg_match('/9/', $val) == 1){
                    $b_arr[] = $val;
                }elseif(preg_match('/6/', $val) == 1 || preg_match('/7/', $val) == 1){
                    $c_arr[] = $val;
                }else{
                    $d_arr[] = $val;
                }
            ...
            if(in_array($jihao,$b_arr)){
                return 2 ;
            }elseif(in_array($jihao,$c_arr)){
                return 3 ;
            }elseif(in_array($jihao,$d_arr)){
                return 4 ;
            }
        }
