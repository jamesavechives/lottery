function creatstatarrhtml(data){
    console.log(data);
    $("#data-tab-statarr .sjp").remove();
    $("#data-tab-statarr-sjp2 .sjp").remove();
    $("#data-tab-statarr-sjp3 .sjp").remove();
    ...
    var fjys = $('#data-tab-statarr');
    var fjys2 = $('#data-tab-statarr-sjp2');
    var fjys3 = $('#data-tab-statarr-sjp3');
    $.each(cpdata,function(key,cp){
        ...
        if(((key+1)%6) == 0){
            ...
            $('#data-tab-statarr').append(fenduan);
        }
        var eid = cp['EID'];
        ...
        for(var i=1; i<6; i++){
            ...
            if(dataexps[eid]['one'][i] == 0){
                tds = "<td class='zx120 zx120sjp "+'heng'+i+" ' data-yuce = '"+dataexps[eid]['one'][i]+"'><span class='zx120span stat_arr_zx120'>"+abcbian(i,jhnu.length)+"</span></td>";
            }else{
                tds = "<td class='zx120 zx120sjp "+'heng'+i+" ' data-yuce = '"+dataexps[eid]['one'][i]+"'><span class='ptspan'>"+dataexps[eid]['one'][i]+"</span></td>";
            }
            $('#data-tab-statarr #trzx120_'+key).append(tds);
            if(i == 6){
                $('.heng6').addClass('qudiaos');
            }
        }

        $('#data-tab-statarr #trzx120_'+key).append("<td class='fqtds'><div class='xian'></div></td>");
       ...
       var iii=0;
        ...
        for(var i=0; i<3; i++){
            ...
            $('#data-tab-statarr-sjp2 #trzx120_'+key).append(tds);
        }
        ...
        for(var i=0; i<3; i++){
            ...
            $('#data-tab-statarr-sjp3 #trzx120_'+key).append(tds);
        }
    ...
    $('.statarr-tjf .count_zx').each(function(key,val){
           $(this).text(dataexps[lasteid]['one-count'][key+1]);
    });
    $('.statarr-tjf .count_sjp').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp-count'][key+1]);
    });
    $('.statarr-tjf .count_sjp2').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp2-count'][key+1]);
    });
    $('.statarr-tjf .count_jl').each(function(key,val){
        $(this).text(dataexps[lasteid]['jl-count'][key]);
    });

    $('.statarr-tjf .pjyl_zx').each(function(key,val){
        $(this).text(dataexps[lasteid]['one-pj-miss'][key+1]);
    });
    $('.statarr-tjf .pjyl_sjp').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp-pj-miss'][key+1]);
    });
    $('.statarr-tjf .pjyl_sjp2').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp2-pj-miss'][key+1]);
    });
    $('.statarr-tjf .pjyl_jl').each(function(key,val){
        $(this).text(dataexps[lasteid]['jl-pj-miss'][key]);
    });

    //最大
    $('.statarr-tjf .zdyl_zx').each(function(key,val){
        $(this).text(dataexps[lasteid]['one-max'][key+1]);
    });
    $('.statarr-tjf .zdyl_sjp').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp-max'][key+1]);
    });
    $('.statarr-tjf .zdyl_sjp2').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp2-max'][key+1]);
    });
    $('.statarr-tjf .zdyl_jl').each(function(key,val){
        $(this).text(dataexps[lasteid]['jl-max'][key]);
    });

    //最大
    $('.statarr-tjf .zdlc_zx').each(function(key,val){
        $(this).text(dataexps[lasteid]['one-max-lc'][key+1]);
    });
    $('.statarr-tjf .zdlc_sjp').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp-max-lc'][key+1]);
    });
    $('.statarr-tjf .zdlc_sjp2').each(function(key,val){
        $(this).text(dataexps[lasteid]['sjp2-max-lc'][key+1]);
    });
    $('.statarr-tjf .zdlc_jl').each(function(key,val){
